"""
Eigenvalue spectra for the time-dependent phase-boundary stability problem.
Toppaladoddi & Wettlaufer (2019) — extended to fully time-dependent.
Domain mapping
--------------
  Liquid  z_l in [0, 1]     →  zeta in [0, 1]   (identity)
  Solid   z_s in [1, 1+d0]  →  zeta in [0, 1]   via z_s = 1 + d0*(1 - zeta)
This aligns both interfaces at zeta = 1 (RIGHT)
and both far walls at zeta = 0 (LEFT).
Perturbation convention:   exp(i(kx + my - sigma*t))
  Im(sigma) > 0  →  unstable
  Re(sigma)/k    →  phase speed
"""
import numpy as np
...
def solve_evp(N):
    ...
    # ── base-state profile (Poiseuille)  u^(0)(z) = 4z(1-z) ──────────────
    ...
    ns = dict(
        ...
        h=h, sigma=sigma, U0=U0, U0z=U0z, U0zz=U0zz,
        dz=dz, lift=lift,
    ...
    problem.add_equation("wz(z=1) - 1j*k*U0z(z=1)*h = 0")   # no-slip  (linearized)
...
ax.annotate(f"({cr[idx]:.2f}, {ci[idx]:.2f})", 
...
ax.set_title(
    f'Poiseuille Melting Phase-Change Spectrum\n'
